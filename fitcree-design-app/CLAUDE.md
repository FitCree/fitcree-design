...
2. `src/components/` で再利用可能なコンポーネントを探す
3. ルールに則ったTailwindクラスを選択する

## 3. 重要な実装ルール

- **既存コンポーネント優先**: `src/components/` 内（特に `forms/elements/`, `common/`）の既存パーツを最優先で再利用
- **ロール別テーマ**: フォーム要素は `src/components/forms/elements/form-theme.ts` の `getFormTheme()` を使ってCreator/Clientのカラーを切り替える。色のハードコード禁止
...
- Styling: Tailwind CSS v4（テーマ設定は `src/app/globals.css` の `@theme inline`）
- Icons: Lucide React
