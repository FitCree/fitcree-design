      ...
      {displayUrls.map((url, idx) => (
        <div key={idx} className="flex gap-2">
          <input
            ...
            className={`flex-1 p-3 border border-neutral-300 rounded-lg text-neutral-800 focus:ring-1 ${theme.ring} placeholder:text-neutral-400`}
          />
          {idx === displayUrls.length - 1 && canAddMore && (
            <button
              onClick={addUrlField}
              type="button"
              className={`px-3 py-2 bg-white border border-neutral-300 rounded-lg text-neutral-600 ${theme.borderHover} text-sm`}
            >
              + 追加
            </button>
          )}
