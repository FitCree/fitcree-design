...
export default function GraphicEditForm({ workId, work }: GraphicEditFormProps) {
  const router = useRouter();
  const MAX_IMAGES = 10;

  // 既存画像をUploadedImage形式に変換
  const initialImages: UploadedImage[] = (work.graphicImages ?? []).map((url, i) => ({
    id: `existing-${i}`,
    preview: url,
    name: `image_${i + 1}.png`,
  }));
  ...
  const [graphicName, setGraphicName] = useState(work.graphicName || '');
  const [industry, setIndustry] = useState(work.industry || '');
  const [graphicType, setGraphicType] = useState(work.graphicType || '');
  const [tools, setTools] = useState<string[]>(work.tools);
  const [graphicTags, setGraphicTags] = useState<string[]>(work.siteTags);
            ...
            >
              <div className="p-6 pb-4 shrink-0">
                <h2 className="text-lg font-bold text-neutral-800 mb-1">
                  <span className="text-orange-500">グラフィック</span> の作品を編集
                </h2>
                <p className="text-sm text-neutral-500 mb-4">
                  画像を追加・削除・並び替えできます（最大{MAX_IMAGES}枚）。
                </p>
              ...
              <div className="px-6 pb-6 shrink-0">
                <div className="p-3 bg-blue-50 border border-blue-100 rounded-lg">
                  <p className="text-xs text-blue-700 leading-relaxed flex items-start gap-1.5">
                    ...
                    <span>1枚目の画像がサムネイルになります。最もインパクトのあるビジュアルを先頭にしましょう。</span>
                  </p>
                </div>
              </div>
            </div>
