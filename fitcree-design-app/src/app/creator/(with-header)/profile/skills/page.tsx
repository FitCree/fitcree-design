          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredSkills.length > 0 ? (
              filteredSkills.map(skill => {
                ...
                const categoryLabel = category?.label || skill.category;
                ...
                return (
                  <div
                    ...
                    className="group bg-white rounded-lg border border-neutral-200 p-6 pb-4 hover:border-orange-300 transition-colors cursor-pointer relative overflow-hidden"
                  >
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                      <div className="flex-1">
                        <div className="flex gap-4 mb-4">
                          <div className="">
                            {/* スキルカテゴリ */}
                            <span className="px-2 py-0.5 bg-slate-700 text-white text-xs font-bold rounded inline-flex items-center gap-1">
                              {CategoryIcon && <CategoryIcon size={12} />}
                              {categoryLabel}
                            </span>
                            {/* スキル名 */}
                            <h3 className="text-xl font-bold text-neutral-800 tracking-tight leading-none mt-2">{skill.name}</h3>
                          </div>
                        ...
                        <div className="flex flex-wrap gap-8">
                          ...
                          <div className="flex-1 min-w-[200px]">
                            <span className="text-xs font-bold text-neutral-500 mb-2 block tracking-widest">作品内訳</span>
                            <div className="flex gap-4">
                              <div className="flex items-center gap-1">
                                <Briefcase size={14} className={skill.works.internal > 0 ? "text-orange-500" : "text-neutral-300"} />
                                <span className="text-xs font-bold text-neutral-500">FitCree内実績</span>
                                <span className={`text-sm font-bold ${skill.works.internal > 0 ? "text-orange-500" : "text-neutral-300"}`}>{skill.works.internal}</span>
                              </div>
                              <div className="flex items-center gap-1">
                                <Globe size={14} className={skill.works.external > 0 ? "text-blue-500" : "text-neutral-300"} />
                                <span className="text-xs font-bold text-neutral-500">外部実績</span>
                                <span className={`text-sm font-bold ${skill.works.external > 0 ? "text-blue-500" : "text-neutral-300"}`}>{skill.works.external}</span>
                              </div>
                              <div className="flex items-center gap-1">
                                <User size={14} className={skill.works.personal > 0 ? "text-green-500" : "text-neutral-300"} />
                                <span className="text-xs font-bold text-neutral-500">自主制作</span>
                                <span className={`text-sm font-bold ${skill.works.personal > 0 ? "text-green-500" : "text-neutral-300"}`}>{skill.works.personal}</span>
                              </div>
                            </div>
          ...
          {isModalOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <div className="relative bg-white w-full max-w-xl rounded-lg shadow-lg overflow-hidden animate-in fade-in zoom-in duration-200">
                <div className="px-6 py-4 border-b border-neutral-200 flex items-center justify-between bg-slate-100/50">
                  <div>
                ...
                <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                  ...
                  {/* 登録注意事項 */}
                  <TipsBox title="登録のポイント" content="スキル・ツールは作品と紐づけることも可能です。カテゴリーは自動判定されます。「HTML」と「CSS」のように個別に登録することを推奨しています。" variant="creator" />

                  {/* スキル名入力 */}
                  <div className="space-y-2">
                    <label className="text-base font-bold text-neutral-700 uppercase tracking-widest ml-1">スキル・ツール名</label>
                    ...
                    <div className="relative mt-2">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const matchedSkill = SKILL_MASTER.find(s =>
                            s.name.toLowerCase() === newName.toLowerCase() ||
                            s.name.toLowerCase().startsWith(newName.toLowerCase())
                          );
                          setFormData({
                            ...formData,
                            name: newName,
                            category: matchedSkill ? matchedSkill.cat : formData.category
                          });
                        }}
                      />
                    </div>

                    {/* スキルサジェスト（master-data.tsに定義されている[SKILL_MASTER]を参照）*/}
                    {!editingSkill && formData.name && (
                      <div className="space-y-2 mt-4">
                        <div className="flex items-center gap-2">
                          <Sparkles size={14} className="text-green-500" />
                          <span className="text-xs font-bold text-green-600 uppercase tracking-widest">もしかして</span>
                        </div>
                          ...
                          {SKILL_MASTER
                            ...
                            .map(s => (
                              <button
                                ...
                                className="px-3 py-1.5 bg-white border border-neutral-200 rounded-lg text-sm font-bold hover:bg-neutral-100 transition-colors flex items-center gap-2"
                              >
                                {s.name}
                              </button>
                            ))
                    ...
                    {/* 自動判定されたカテゴリーの表示（master-data.tsに定義されている[SKILL_MASTER]を参照）*/}
                    {formData.name && (() => {
                      const category = SKILL_CATEGORIES.find(c => c.id === formData.category);
                      const CategoryIcon = category?.icon;
                      return (
                        <>
                          <p className="flex items-center gap-2 mt-4 ml-1">
                            <span className="text-sm font-bold text-neutral-500 uppercase tracking-widest">自動判定カテゴリー</span>
                            <span className="px-2 py-0.5 bg-slate-700 text-white text-xs font-bold rounded inline-flex items-center gap-1">
                              {CategoryIcon && <CategoryIcon size={12} />}
                              {category?.label || 'Other'}
                            </span>
                          </p>
                          <p className="text-xs text-neutral-700 font-medium mt-2 ml-1">※カテゴリーは自動適用されます</p>
                        </>
                      );
                    })()}
                  </div>

                  {/* 熟練度（master-data.tsに定義されている[SKILL_LEVEL_LABELS]を参照）*/}
                  <div className="space-y-2">
                    <label className="text-base font-bold text-neutral-700 uppercase tracking-widest ml-1">得意度</label>
                  ...
                  {editingSkill && (
                    <div className="p-4 bg-slate-100 rounded-lg space-y-3 border border-neutral-200">
                      <label className="text-base font-bold text-neutral-700 tracking-widest block">作品内訳</label>
                      <div className="grid grid-cols-3 gap-3">
                        <div className="bg-white p-3 rounded-lg border border-neutral-200 text-center">
                          <div className="text-xs font-bold text-neutral-500 mb-1 flex items-center justify-center gap-1"><Briefcase size={12} className="text-orange-500" /> FitCree内実績</div>
                          <div className="text-lg font-bold text-orange-600">{editingSkill.works.internal}</div>
                        </div>
                        <div className="bg-white p-3 rounded-lg border border-neutral-200 text-center">
                          <div className="text-xs font-bold text-neutral-500 mb-1 flex items-center justify-center gap-1"><Globe size={14} className="text-blue-500" />外部実績</div>
                          <div className="text-lg font-bold text-blue-600">{editingSkill.works.external}</div>
                        </div>
                        <div className="bg-white p-3 rounded-lg border border-neutral-200 text-center">
                          <div className="text-xs font-bold text-neutral-500 mb-1 flex items-center justify-center gap-1"><User size={12} className="text-green-500" /> 自主制作</div>
